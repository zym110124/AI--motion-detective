...
% foreground objects, and displaying results.
    ...
    function obj = setupSystemObjects()    %%%这个function返回的就是一个obj的结构数组
        % Initialize Video I/O
        % Create objects for reading a video from a file, drawing the tracked
        % objects in each frame, and playing the video.

        % create a video file reader
        obj.reader = vision.VideoFileReader('movie.avi');
...
% The state consists of information used for detection to track assignment（跟踪任务）,
...
% the frames.        
    ...
    function tracks = initializeTracks()  %%%返回的是一个空数组
...
% remove noisy pixels and to fill the holes in the remaining blobs.  
    ...
    function [centroids, bboxes, mask] = detectObjects(frame)
        ...
        mask = imfill(mask, 'holes');  %%效果可以见help，大概就是把一个封闭图像中的那些该白的地方都变白了

        % perform blob analysis to find connected components
        [~, centroids, bboxes] = obj.blobAnalyser.step(mask); %%关于[~,的解释见0512的笔记。
    end
...
% current frame, and update its bounding box accordingly.
    ...
    function predictNewLocationsOfTracks()  %%这个函数其实是更新了tracks里面的centroid的值  
        for i = 1:length(tracks)     %%%更新的，我猜想是每一个目标的一些特征值
            bbox = tracks(i).bbox;

            % predict the current location of the track
            predictedCentroid = predict(tracks(i).kalmanFilter);   %%%猜测predict也是一个函数吧，predict就是用来运算kalman滤波的程序

            % shift the bounding box so that its center is at 
            % the predicted location
            predictedCentroid = int32(predictedCentroid) - bbox(3:4) / 2;  %%？？为什么这里是减，还没有弄清楚
            tracks(i).bbox = [predictedCentroid, bbox(3:4)];
        end
    end

%% Assign Detections to Tracks
% Assigning object detections（分配对象检测） in the current frame to existing tracks（现存的） is
% done by minimizing cost（最小成本）. The cost is defined as the negative
...
% cost takes into account the Euclidean distance（欧氏距离） between the predicted
...
% |vision.KalmanFilter|. This value must be tuned experimentally（实验的，实际的）. Setting 
% it too low increases the likelihood of creating a new track, and may
% result in track fragmentation（破裂）. Setting it too high may result in a
% single   %%%这里说的value是什么意思还要有待观察
% track corresponding to a series of separate moving objects.   
%
% The |assignDetectionsToTracks| function uses the Munkres' version（曼克莱斯算法） of the
% Hungarian algorithm（匈牙利算法） to compute an assignment which minimizes the total
% cost. It returns an M x 2 matrix containing the corresponding indices of
% assigned tracks and detections in its two columns. It also returns the
% indices of tracks and detections that remained unassigned（为指定的）. 

    function [assignments, unassignedTracks, unassignedDetections] = ...
            detectionToTrackAssignment()

        nTracks = length(tracks);     %%返回的是当前跟踪的tracks的个数
        nDetections = size(centroids, 1);   %%返回的的当前detections的个数。size返回的是行值
        ...
        [assignments, unassignedTracks, unassignedDetections] = ...
            assignDetectionsToTracks(cost, costOfNonAssignment);   %%help中有该函数的具体用法以及示例
    end
...
% visible count by 1. Finally, the function sets the invisible count to 0. 
    ...
    function updateAssignedTracks()
        numAssignedTracks = size(assignments, 1); %%返回的是已经配好对的detection和track
        for i = 1:numAssignedTracks
            trackIdx = assignments(i, 1);   %%这一行和下面一行分别是track和detection的序号
            ...
            correct(tracks(trackIdx).kalmanFilter, centroid);  %%这也是kalman中的一个常用函数用来修正用的。
            ...
            % update track's age
            tracks(trackIdx).age = tracks(trackIdx).age + 1;    %%%这是将每一个track都分了出来，在所有的帧里面，计算出这一个track的次数
...
% %%没有detection匹配的track  
    ...
    function updateUnassignedTracks()
        for i = 1:length(unassignedTracks)
            ...
            tracks(ind).age = tracks(ind).age + 1;   %%age是自track第一次被发现后的帧的数量
...
% that have been invisible for too many frames overall. 
    ...
    function deleteLostTracks()
        ...
        % compute the fraction（部分、分数） of the track's age for which it was visible
        ages = [tracks(:).age];  %%是age中所有的数
        totalVisibleCounts = [tracks(:).totalVisibleCount];
        visibility = totalVisibleCounts ./ ages;

        % find the indices(索引) of 'lost' tracks
        lostInds = (ages < ageThreshold & visibility < 0.6) | ...
            [tracks(:).consecutiveInvisibleCount] >= invisibleForTooLong; %%%这个函数返回的是索引值

        % delete lost tracks
        tracks = tracks(~lostInds);  %%这个命令显然就是删除了这个lostInds索引的track
    end

%% Create New Tracks
% Create new tracks from unassigned detections. Assume that any unassigned
% detection is a start of a new track. In practice, you can use other
% cues（线索）
% to eliminate noisy detections, such as size, location, or appearance.

    function createNewTracks()   %%？？这样的function是不是函数里面得到的值是可以作为全局变量使用的，因为也没有returun，没有输入，输出？
        centroids = centroids(unassignedDetections, :);  %%这里面的unassignedDetections都是序号，这里大概是一下子把所有的都返回成一个新的centroids的数组了。下一行是同样的道理
        bboxes = bboxes(unassignedDetections, :);

        for i = 1:size(centroids, 1)   %%返回所有的新的detection的数目
            ...
            kalmanFilter = configureKalmanFilter('ConstantVelocity', ...
                centroid, [200, 50], [100, 25], 100);   %%help中有该函数的用法
...
% displays the frame and the mask in their respective video players. 
    ...
    function displayTrackingResults()
        ...
        frame = im2uint8(frame);   %%把图像数据转为无符号8位整型
        mask = uint8(repmat(mask, [1, 1, 3])) .* 255;  %%repmat是复制函数  unit8表示无符号整型

        minVisibleCount = 8;  %%这是一个判断阈值，只有当某一个tracks存在的帧数超过它，才可以显示
        if ~isempty(tracks)
            ...
            reliableTrackInds = ...
                [tracks(:).totalVisibleCount] > minVisibleCount;  %%%返回的还是索引值，而且应该是所有的索引值
            ...
            if ~isempty(reliableTracks)
                % get bounding boxes
                bboxes = cat(1, reliableTracks.bbox);   %%cat函数见help是创建出一个新的连接数组
                ...
                isPredicted = cell(size(labels));   %%cell(N)创造一个N*N的数组
                isPredicted(predictedTrackInds) = {' predicted'};
                labels = strcat(labels, isPredicted);  %%这个函数就是把labels和isPredicted连在一起了

                % draw on the frame
                frame = insertObjectAnnotation(frame, 'rectangle', ...
                    bboxes, labels);  %%该函数用法见help
